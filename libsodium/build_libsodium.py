...
def main():
    ...
    for compiler in COMPILERS:
        for option in compiler["options"]: 
            ...
            subprocess.run(["make", "clean"], cwd=Path("libsodium-stable/").resolve())
            subprocess.run(["./configure", "--disable-shared"],cwd=Path("libsodium-stable/").resolve(), env=env)
            build_res = subprocess.run(["make", "-j"], cwd=Path("libsodium-stable/").resolve(), env=env)
            if build_res.returncode == 0:
                shutil.move(Path("libsodium-stable/src/libsodium/.libs/libsodium.a"), Path("precompiled/libsodium.%s_%s.a" % (compiler["name"], option)))        
